...
<div class={cn('space-y-4', className)}>
	...
	<div class="space-y-2">
		...
		<div class="relative">
			...
			>
				<span class="text-muted-foreground">
					...
					{:else}
						{_llmProviders.map((p) => p.charAt(0).toUpperCase() + p.slice(1)).join(', ')}
					{/if}
				</span>
			...
			{#if showLlmDropdown}
				<div
					class="absolute z-50 mt-1 w-full rounded-md border bg-popover py-1 shadow-md"
					use:clickOutside={() => (showLlmDropdown = false)}
				>
					<!-- Search input -->
					<div class="px-3 pb-2">
						<Input
							bind:value={llmSearchInput}
					...
					<div class="max-h-48 overflow-auto">
						{#each filteredLlmProviders as provider}
							<button
								type="button"
								class="flex w-full cursor-pointer items-center gap-2 px-3 py-2 text-left text-sm outline-none hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground"
								onclick={() => toggleLlmProvider(provider.value)}
							>
								<div class="flex h-4 w-4 items-center justify-center rounded border">
									{#if _llmProviders.includes(provider.value)}
						...
						{/each}
						{#if filteredLlmProviders.length === 0}
							<div class="px-3 py-2 text-sm text-muted-foreground">No providers found</div>
						{/if}
					</div>
				</div>
			{/if}
		...
		<!-- Selected LLM Providers as badges -->
		{#if _llmProviders.length > 0}
			<div class="mt-2 flex flex-wrap gap-2">
				{#each _llmProviders as provider}
					<span
						class="inline-flex items-center gap-1 rounded-full bg-green-100 px-3 py-1 text-sm font-medium text-green-800 transition-colors hover:bg-green-200 dark:bg-green-900 dark:text-green-300 dark:hover:bg-green-800"
					>
						{provider}
						<button
							type="button"
							class="ml-1 cursor-pointer rounded-full p-0.5 hover:bg-green-900/20 focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:outline-none"
		...
		<p class="text-xs text-muted-foreground">
			Select which AI services can be used with this prompt.
		</p>
	</div>
